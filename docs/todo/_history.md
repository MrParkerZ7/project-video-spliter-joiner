...
## 2026-08-22 · G-037 drive — T-100 done
- **T-100 done** (`d45bc1a`, +12 → 683): BulkCutViewModel.SelectedItem + shared Player (one FfmeMediaPlayer, new optional ctor arg). Open-on-select → Player.Open/Unload (MediaReopenGuard is INSIDE FfmeMediaPlayer, auto-engaged — key deviation; no VM-level guard). Lifecycle: stop-on-run, unload-on-clear, auto-select first row, stale-row guard (remove→neighbour), MainViewModel.StopInactiveScreenPlayers on tab change. Note for T-101: pane code-behind must attach the Bulk MediaElement via Player.PlayerControl on Loaded (else Open/Stop are silent no-ops); set-at-playhead command belongs in T-101. T-101 unblocked; T-103 still needs T-102.

## 2026-08-22 · G-037 drive — T-102 done
- **T-102 done** (`05ca966`, +26 → 709): CutProfile record → Core (src/Core/Profiles/, WPF-free, throwing validation, name trimmed = upsert key). IAppSettings.CutProfiles/SaveProfile(upsert-by-name)/DeleteProfile + SettingsDto List<CutProfileDto> (name+introSeconds+outroSeconds?), backward-compat (absent field→empty, malformed skipped, empty→null omits key). CutProfileApplier (static App helper, WPF-free): ApplyProfile (intro-absolute clamped + outro-from-end + re-snap/re-validate + reuses ApplyToAllReport) + BuildProfileFromRow. Updated 4 IAppSettings test fakes. Both G-037 roots done; T-103 unblocked. T-103 calls settings.CutProfiles/SaveProfile/DeleteProfile + CutProfileApplier.ApplyProfile/BuildProfileFromRow.

## 2026-08-22 · G-037 drive — T-101 done
- **T-101 done** (`fa6d97a`, +6 → 715): Bulk preview pane. RootGrid 3→5 rows: header · preview pane (*) · GridSplitter · row list (ListBox, *) · footer. Pane reuses PlayerView (jog ±1s/5s/10s/20s/1m/5m, scrub, hover-thumb, frame-step, play/pause) — MediaElement attaches automatically via PlayerView.xaml.cs OnLoaded (Bulk's own FfmeMediaPlayer), no code-behind needed. Two set-at-playhead commands on the WPF-free VM (SetIntroAtPlayhead/SetOutroAtPlayhead, CanSetCutAtPlayhead guard, HeroButton). Row list → ListBox w/ SelectedItem two-way + gold selection ring (no blue chrome). Deviation: no TimelineView (PlayerView scrub serves as timing bar — was optional; Bulk Player has no Timeline sub-VM). T-103 unblocked (T-104 still needs it).
...
## 2026-08-22 · todo-task — G-038 planned (Bulk Cut profile thumbnails + cut-point frame thumbnails)
- User ask: cut profiles carry an optional user-uploaded thumbnail (recognizable for reuse) + frame thumbnails at each cut point (quick-check the cut). Grounded reuse: CutProfile+DTO (T-102), profiles-bar Save popup (T-103), IThumbnailService.GetThumbnailAsync + ThumbnailPreviewViewModel debounce, BulkItemViewModel IntroEnd/OutroStart Snapped handles.
- Decisions: profile thumb = FILE under %LOCALAPPDATA%/profile-thumbs (path in settings, not base64); auto-default = source row's intro-end frame on save (upload overrides, clear→placeholder); cut-point thumbs on every row, both handles, debounced+best-effort at snapped time.
- **G-038** + T-106..109: T-106 CutProfile.ThumbnailPath + persistence + ProfileThumbnailStore [M,now] · T-107 profile-thumb UI (auto-default/upload/clear/picker) [M,←T-106] · T-108 per-row cut-point frame thumbnails [L,now] · T-109 docs+spec update [S,←T-107+T-108]. Two proceedable roots (T-106, T-108). proceed-with: perfect-dev --auto-approve. Next: todo-next-all.

## 2026-08-22 · todo-task — T-110 planned (Split jog buttons should freeze, not play ~1s)
- Bug: Split preview jog buttons (−1s/−5s/…) play ~1s after each move instead of freezing on the target frame. Root cause: PlayerViewModel.SkipBy→Scrub→FfmeMediaPlayer.Seek calls _element.Seek() WITHOUT pausing, so FFME renders a seek burst; StepFrame already pauses-first (FfmeMediaPlayer:345) — jog doesn't. Fix: make jog a freeze gesture (pause-first + seek, mirror StepFrame) so it sits paused on the exact frame = Player.Position = where SetCutAtPlayhead cuts (snapped as today). Scope freeze to jog/step, not general click-to-seek. Single-module fix → one reviewed task (serves: none), not an epic. serves-spec: SPEC-013 (jog-freeze invariant). Ready/now. Next: todo-next-all or todo-task-next.

## 2026-08-23 · T-110 done — Split jog buttons freeze on the frame
- **T-110 done** (`49c02ad`, +2 → 821): jog/skip now FREEZES the preview on the exact frame (no ~1s play burst). Fix = pause-then-seek at PlayerViewModel.Scrub (the chokepoint every jog funnels through: SkipBy/JumpToStart/JumpToEnd/SeekToMarker) — if IsPlaying, _player.Pause() then clear IsPlaying then BeginSeek; mirrors StepFrame. Play-from-here (scrub drag, timeline click) go via the Position setter/BeginSeek, NOT Scrub, so they stay live/un-frozen. Jog lands paused at exact ±Ns = Player.Position = cut point (snapped as today). Option (b) VM-Pause chosen over a new IMediaPlayer.FreezeSeek (would churn 11 fakes). 2 serves-spec:SPEC-013 tests. app-run verifies the render.

## 2026-08-23 · G-038 drive — T-106 done
- **T-106 done** (`7d3c411`, +29 → 850): CutProfile.ThumbnailPath (optional 4th param, pure Core, null-normalized) + AppSettings DTO round-trip (absolute path, backward-compat absent→null, WhenWritingNull byte-clean, no image bytes in JSON) + ProfileThumbnailStore (copy into %LOCALAPPDATA%/profile-thumbs, safe-name + SHA-256 collision guard, ext-preserving, best-effort Delete/DeleteByPath, DefaultRoot mirrors thumb-cache) + DeleteProfile cascade (store injected via optional AppSettings ctor arg, IAppSettings unchanged). CutProfileApplier unaffected. T-107 unblocked. T-107 calls: store.Save(name,img)→path → SaveProfile(profile with {ThumbnailPath=path}); clear→store.Delete+null; display profile.ThumbnailPath (missing file = no thumbnail).

## 2026-08-23 · G-038 drive — T-107 done
- **T-107 done** (`8eeae8c`, +7 → 857): profile-thumbnail UI. SaveProfileWithAutoThumbnailAsync — saves first (never blocked), then best-effort grabs the intro-end frame (Thumbnails.GetThumbnailAsync at IntroEnd.Snapped) → store.Save → folds path onto the profile → re-persist; null/throw → placeholder. Upload image… (OpenFileDialog, code-behind) + Clear on the selected profile. ComboBox ItemTemplate [28px thumb chip] Name (reused existing PathToBitmapConverter — OnLoad/IgnoreImageCache/Freeze, null/missing-safe; no dup converter). WPF-free VM. 7 serves-spec:SPEC-007 tests. T-109 still needs T-108.
...
## 2026-08-23 · G-039 drive — T-111 done (apply-to-all re-activation, root-caused)
- **T-111 done** (`7f98314`, +11 → 875): root cause = candidate 3 (CanExecute stuck) — RelayCommand.CanExecuteChanged forwarded ONLY to CommandManager.RequerySuggested; RaiseCanExecuteChanged just called InvalidateRequerySuggested → NO deterministic notify to bound buttons (raised=0 proven). Apply→all went stale after first use (re-evaluated only on incidental UI input). Fix (true layer, app-wide + additive): RelayCommand now raises a private _canExecuteChanged event directly (still chains RequerySuggested); BulkCutViewModel.RaiseProfileCommandStates re-raises each profile command explicitly + RaiseRunState re-raises ApplyProfileToAll. Candidates 1/2/4 empirically ruled out (9 guard tests pass unchanged). CutMarkerViewModel untouched (Split markers safe). 11 serves-spec:SPEC-011 tests. T-114 still needs T-112/T-113.

## 2026-08-23 · G-039 drive — T-112 done (Bulk layout-mode-aware)
- **T-112 done** (`d24b4d6`, +4 → 879): BulkCutView RootGrid 5→3 rows (header · OrientedSplitPanel · footer). FirstChild=preview pane, SecondChild=row-list; direct IsVertical bind (no InverseBool, mirrors Split's OUTER panel) → vertical mode stacks pane above list, horizontal = side-by-side. Bulk-SPECIFIC ratio pair BulkHorizontalSplitRatio/BulkVerticalSplitRatio added to IAppSettings/AppSettings/DTO (backward-compat, ClampRatio, defaults 0.4/0.5) + MainViewModel + TwoWay bind — no cross-tab coupling with Split. Preview player survives re-parent (OrientedSplitPanel.Rebuild only reassigns Grid.Row/Col, never Children.Clear → no MediaElement re-attach). Pure View re-parent, no Bulk VM change. 4 serves-spec:SPEC-011 ratio round-trip tests. T-114 still needs T-113.
...
## 2026-08-23 · todo-task — G-040 planned (Bulk Cut fixes: slow selection + apply-to-all discoverability)
- User ask (2 bugs): (1) "where's apply to all button?"; (2) row selection extremely slow / sometimes doesn't work. Grounded: (1) both apply-to-all exist but compact — per-row ⧉ (BulkCutView.xaml:713, ApplyToAllCommand) + profile "⧉ all" (:260) — discoverability, not missing; (2) SelectedItem setter → OpenOrUnloadSelected → Player.Open(item.Path) (:559) on EVERY select, NO debounce → one FFME decoder-init per row + thrash on rapid select.
- **G-040** + T-115..117: T-115 debounce+defer the SelectedItem→preview-open (instant highlight, latest-wins, ~200-300ms) [M,now,PERF] · T-116 apply-to-all discoverability (visible+labelled both modes) [S,now,UX] · T-117 docs+SPEC-011 [S,←115/116]. auto-scaled to epic (2 independent bugs). proceed-with: perfect-e2e --auto-approve.

## 2026-08-24 · G-040 drive — T-115 done (slow row selection fixed)
- **T-115 done** (`ec48b7f`, +4 → 883): SelectedItem setter now raises HasSelection/command states SYNCHRONOUSLY (instant highlight), only Player.Open is deferred behind a 250ms latest-wins debounce (CTS-swap CancelPendingOpen + OpenAfterDebounceAsync, ctor seam selectionOpenDebounce/selectionOpenDelay mirroring T-108). Arrowing N rows → 1 Open of the settled row (no FFME thrash). Null/clear cancels pending open before Unload; RunBatch cancels pending open before Stop. MediaReopenGuard untouched (last-line safety). T-100 preview tests kept green via injected Immediate delay (adjusted seam, not assertions). 4 serves-spec:SPEC-011/013 tests. T-116 next.
...
## 2026-09-05 · todo-task ×3 — footer polish, profile bar, and Split's missing delete-originals
- **T-160** (S→M, now, high) — Bulk Cut footer: eight controls + three live notes + Run on ONE row. T-156's WrapPanel stopped it CLIPPING but it still reflows into a narrow `*` column while `RunScopeSummary` wraps in its own — cramped by accretion, one ticket at a time. Plan: split the footer Grid into two rows (options full-width on top, `[Delete originals] … [Run]` below, keeping T-146's opposite-ends separation), and group the options by consequence so the three destructive controls read as one block instead of interleaving with neutral checkboxes. View-only, zero binding changes. Pinned by a **window-relative** layout test (panel-relative passes against the very bug — T-156's first version did).
- **T-161** (M, now, medium) — profiles: `ComboBox` → horizontally scrollable bar of clickable chips. Profiles carry pictures (T-129/T-135 exist for that) and a closed dropdown hides every one of them. Reuses the SAME two bindings (`Profiles` / `SelectedProfile`), so every command and gate is untouched; the existing Apply-to-selected / Apply-to-all pair stays the action half. **Decided: a click SELECTS, never applies** — apply-to-all rewrites every ticked row's cut points and must not be one misclick away on a scrolling strip.
- **G-052** + T-162/T-163/T-164 — *"on Split also add auto remove original and auto clear bin same as bulk cut"*. Checked first: **Split has none of it**, manual or auto — `IOriginalDisposer`/`DeleteOriginals`/`AutoDeleteOriginals` live only in Bulk. T-156 built its auto layer deliberately ON TOP of the manual sweep ("one deletion path"), so the auto-only shortcut would mean a hidden second deletion implementation. Hence T-162 (manual sweep) → T-163 (the two checkboxes + all four T-156 arming gates) → T-164 (terminal perfect-docs).
- **The part that is NOT a copy of Bulk:** Bulk deletes one original per row against one output; Split deletes one source that produced **N parts**, so eligibility is "are ALL parts on disk and non-empty?" — binning a 4 GB source when part 4 of 6 failed loses footage that exists nowhere else. That invariant is what G-052 hangs on.
