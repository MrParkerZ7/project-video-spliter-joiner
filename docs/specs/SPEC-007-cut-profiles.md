...
- **I69** — every explicit-upload failure is **reported to the user** on the screen's existing error surface (`Operation.Error`, set through the additive `OperationViewModel.ReportFailure` — no dialog, no new surface), each with its own headline, an actionable `Hint`, and a copyable `RawTail` carrying the chosen path plus the refusing exception's message. The five reportable outcomes are distinct (`ThumbnailAttachOutcome`): `NoProfile` (nothing selected), `NoImageChosen` (null/blank path), `ProfileNotSaved` (the name has no persisted profile), `ImageUnreadable` (the store refused the source — `FileNotFoundException`/`DirectoryNotFoundException`/`ArgumentException`), `StoreFailed` (any other store failure — unwritable root, locked target, I/O error).
- **I70** — a failed upload does **no extra work**: it returns before the profile upsert, so there is no `SaveProfile` write and no `RefreshProfiles` re-projection (the bar's `SelectedProfile` stays the SAME instance, not a re-created record), and no frame grab is attempted (the explicit path never touches `IThumbnailService`). The only I/O is the single `ProfileThumbnailStore.Save` call that refused, so the profile's recorded `ThumbnailPath` and the file it points at both survive a refusal that happens **before** the store starts writing (blank/missing source — I49's guard; unwritable root — the `Directory.CreateDirectory` step).
  - *Known store-side gap (not an upload-path behaviour):* `ProfileThumbnailStore.Save` deletes the profile's prior thumbnail **before** copying the new one in (I45), so a copy that fails *after* that point (e.g. a source locked by another process, with a writable root) leaves the profile's `ThumbnailPath` pointing at a file that is already gone. The upload still reports (`StoreFailed`), and the profile record is still untouched, but the picture reverts to the placeholder. Fixing it means making `Save` copy-then-swap — a `ProfileThumbnailStore` change, out of scope for T-129.
...
- Tests: `tests/Core.Tests/CutProfileTests.cs` · `tests/App.Tests/CutProfilePersistenceTests.cs` · `tests/App.Tests/CutProfileApplierTests.cs` · `tests/App.Tests/ProfileThumbnailStoreTests.cs` (store + `DeleteProfile` cascade, T-106) · `tests/App.Tests/BulkCutProfileThumbnailTests.cs` (auto-default/upload/clear, T-107; upload-failure reporting, T-129) (and app-layer `tests/App.Tests/BulkCutProfileCommandsTests.cs`)
